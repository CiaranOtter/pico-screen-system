...
async def connect_wifi():
    ...
    ssid     = config.get('wifi_ssid')     or 'ZyXEL_C26D'
    password = config.get('wifi_password') or 'R73C7P8LAT'
            ...
            if state.btn_sequence:
                ...
                if item.get('hasSaved'):
                    try:
                        ...
                        print(f"Loading slot {slot_idx}, RAM: {mem.free()}")

                        try:
                            buf = bytearray(240 * 240 * 2)
                        except MemoryError:
                            print("Cannot alloc slot buffer")
                            state.render_state()
                        ...
                        try:
                            with open(path, 'rb') as f:
                                ...
                                pos   = 0
                                while pos < len(buf):
                                    chunk = f.read(min(CHUNK, len(buf) - pos))
                                    if not chunk:
                                        break
                                    n = len(chunk)
                                    buf[pos:pos + n] = chunk
